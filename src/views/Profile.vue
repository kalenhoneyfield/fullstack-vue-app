<template>
  <b-container fluid class="mx-auto text-center" @click="fillit">
    {{ name }}
    <hr />
    {{ email }}
    <hr />

    <div class="progress" :data-percentage="percentageFilled">
...
export default {
  ...
  methods: {
    fillit() {
      this.percentageFilled += Math.floor(Math.random()) + 1;
      this.percentageFilled = this.percentageFilled > 100 ? 90 : this.percentageFilled;
...
@for $i from 1 through $howManySteps {
  ...
  @if $i > ($howManySteps/2) {
    .progress[data-percentage='#{$stepName}'] {
      .progress-right .progress-bar {
        animation: loading-#{1 * ($howManySteps/2)} $animationTime linear forwards;
      }
      .progress-left .progress-bar {
